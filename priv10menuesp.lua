...
-- Freecam module (inlined from freecam.lua)
    local freecam = {}
    ...
    do
        ...
        local PlayerState = {} do
            ...
            function PlayerState.Pop()
                ...
                -- Force the mouse to unlock when leaving freecam so it never gets
                -- stuck locked. Restoring the pre-freecam saved value can leave it
                -- locked (e.g. if the game locked it before freecam started).
                UserInputService.MouseIconEnabled = true
                UserInputService.MouseBehavior = Enum.MouseBehavior.Default
                mouseBehavior = nil
        ...
        local function StopFreecam()
            ...
            -- Unlock guard: some games re-lock the mouse every frame, which can
            -- re-lock it the instant freecam stops. Force Default for a short
            -- window after disabling so the mouse always ends up free.
            task.spawn(function()
                for _ = 1, 45 do
                    task.wait()
                    UserInputService.MouseBehavior = Enum.MouseBehavior.Default
        ...
        -- Toggle logic: menu keybind + Shift+P both route through set_active
        ...
        -- Listen for Shift + P using UserInputService (doesn't get blocked by ContextActionService)
...
-- Library functions 
    -- Misc functions
            ...
            for propertyName, objects in next, themes.utility[theme] do
                for _, object in next, objects do
                    if object:GetAttribute("PrivToggleState") == false and propertyName == "BackgroundColor3" then
                        -- keep toggles that are off visually off (unfilled) even when the theme changes
        ...
        -- Visuals registry and helper APIs
        ...
        -- Register a visuals page. center_section/right_section may be pre-created sections
        ...
        function library:set_visuals_page(id)
            ...
            local function set_visible(obj, visible)
                ...
                -- If obj itself is a section object (has .frame), treat as single
                if type(obj) == "table" and obj.frame then
                    obj.frame.Visible = visible
                    return
                end

                -- Otherwise assume it's a list/array of section objects
        ...
        function library:update_keybind_visualizer()
            ...
            -- Collect ALL active keybinds (menu key excluded)
            local actives = {}
    ...
    -- Library element functions
        ...
        function notifications:refresh_notifs()
            ...
            -- Collect numeric keys and sort to ensure deterministic ordering
            local idxs = {}
        ...
        function notifications:create_notification(options)
            ...
            -- Instances
                ...
                local outline = library:create("Frame", {
                    Parent = library.sgui;
                    Position = dim_offset(baseX, baseY + wmHeight + ((#notifications.notifs + 1) * gap)); -- align with watermark left, keep consistent gap
                    BorderColor3 = rgb(0, 0, 0);
                    Size = dim2(0, 0, 0, 24);
                    BorderSizePixel = 0;
	...
	fpsLabel.Text = "FPS: " .. tostring(math.floor(realFPS))
        ...
        function library:section(properties)            
            ...
            -- Instances
                ...
                if cfg.autofill == false then 
                    ...
                    -- mark this accent as autofill so we can find it later
                    local marker = Instance.new("BoolValue")
                    marker.Name = "__autofill"
                    marker.Value = true
                    marker.Parent = accent

                    -- initial pixel height (fallback)
                    accent.Size = dim2(1, 0, 0, 24)

                    -- helper to find UIListLayout in the column
                    ...
                    -- compute remaining height and apply to this accent if it's the bottom-most autofill
                    local function apply_autofill()
                        ...
                        -- find bottom-most autofill accent by largest AbsolutePosition.Y
                        local lastAutofill
                    ...
                    -- wire up updates: column size and layout content size
                    local col = accent.Parent
                    local layout = find_listlayout(col)
                    if layout then
                    ...
                    -- also react when this accent's AbsoluteSize changes (content may expand/collapse)
        ...
        -- Elements  
            ...
            function library:list(options)
                ...
                -- Functions
                    ...
                    local function update_label()
                        if cfg.open then
                            -- prefer currently highlighted option (white text)
            ...
            function library:dropdown(options) 
                ...
                -- Connections 
                    dropdown_holder.MouseButton1Click:Connect(function()
                        ...
                        -- position accent flush under the dropdown button
            ...
            function library:init_config(window) 
                ...
                local misc_left = misc_tab:column({}) -- left-side misc column
                local misc_center = misc_tab:column({}) -- center column (fov changer)
                ...
                movement_section:slider({name = "speed", flag = "speedhack_speed", min = 12, max = 500, default = 16, interval = 1, suffix = ""})
...
-- Using the locally defined library implementation instead of the remote library override
...
library:connection(uis.InputBegan, function(input, gameProcessed)
    -- Don't ignore inputs that CoreGui or the game processed; capture toggle keys regardless.
...
local esp -- forward declaration, assigned once the ESP module (bottom of file) initializes
...
for _, page in next, pages do
    -- create page placeholders (center + right)
    if page.id == "scientist" then
        -- create two sections in each column with random names and add example elements
        ...
        -- register the page with the visuals registry; store and index the returned entry
        local page_entry = library:register_visuals_page(page.id, page.label, page_data.center, page_data.right)
        visuals_pages[page.id] = page_entry
    end
end

-- open the default page by id
-- populate the player page center and right columns to match the middle/right layout
do
    ...
    if player_page and player_page.center and player_page.right then
        ...
        -- Options (right column) -- flags map directly onto the ESP module's flags table (see bottom of file)
        ...
        -- model highlight (instance chams): off / ontop (always-on-top) / occluded (through walls)
        -- picking ontop/occluded reveals the fill + outline color pickers below
        ...
        -- start hidden (model defaults to "off")
...
-- FOV changer (RunService.Stepped for lowest latency)
run.Stepped:Connect(function()
    ...
    -- zoom key overrides the fov value while held
...
-- Movement (speedhack + no jump restrictions)
local speedhack_original_walkspeed = 16
local speedhack_was_active = false
...
run.Stepped:Connect(function()
    ...
    if speedhack_active then
        if not speedhack_was_active then
            speedhack_original_walkspeed = humanoid.WalkSpeed
            speedhack_was_active = true
        end
        humanoid.WalkSpeed = flags["speedhack_speed"] or speedhack_original_walkspeed
    elseif speedhack_was_active then
        humanoid.WalkSpeed = speedhack_original_walkspeed
        speedhack_was_active = false
    end

    -- no jump restrictions
    ...
    if no_jump_active then
        ...
        -- infinite jump: force the Jumping state while space is held so you can
        -- re-jump in mid-air with no cooldown
        ...
        if is_valid_aimbot_target(player) and player.Character then
            ...
            if target_part then
                ...
                if rootpart then
                    ...
                    -- WorldToScreenPoint is relative to the 3D viewport, which is offset
                    -- down by the top bar (GUI inset). Shift into absolute screen space so
                    -- detection lines up with the FOV circle (drawn at ViewportSize / 2).
    ...
    if aimbot_enabled and is_aim_key_active() then
        ...
        if target and target.Character then
            ...
            if part and not aimbot_silent then
                ...
                local new_cframe = CFrame.new(camera_pos, target_position)
...
-- ESP (ported as-is from esp/esp.lua, logic/projection untouched; wired to the menu's `flags` table above)
    local bones = {
        -- R15 skeleton
        {"Head", "UpperTorso"},
    ...
    -- fill in esp flags the menu doesn't expose a control for, without touching anything the UI already set
    ...
    -- model highlight (instance chams) defaults
        ...
        -- Functions 
            ...
            function esp:create_object( player )
                ...
                do --[[ data functions ]]
                    ...
                    data.refresh_descendants = function(character)
                        ...
                        -- Instance highlight (chams) -- parented to the character so it respawns with it
                ...
                do --[[ init / connections ]]  
                    data.refresh_descendants(player.Character)
                    ...
                    player.CharacterAdded:Connect(function(character)
                        data.refresh_descendants(character)
                    end)
            ...
            function esp.refresh_elements( )
                for _,v in players:GetPlayers() do 
                    ...
                    -- Model highlight (instance chams)
            ...
            esp.connection = run.RenderStepped:Connect(function()
                ...
                -- shared highlight helper: keeps the instance chams in sync with the per-frame gating
                ...
                for _, player in players:GetPlayers() do 
                    ...
                    -- player is visible and within distance: re-enable the instance chams
